...
from hcode.conversation import (
    ConversationManager,
    Message,
    ToolResultBlock,
    ToolUseBlock,
...
from hcode.memory.instructions import (
    MAX_INCLUDE_DEPTH,
    load_instructions,
    process_includes,
)
...
class TestLoadInstructions:
    def test_single_layer(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        hcode_md = tmp_path / "HCODE.md"
        hcode_md.write_text("project instructions", encoding="utf-8")
    ...
    def test_multi_layer_priority(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        """同一目录下 HCODE.md 在前，.hcode/HCODE.md 在后（优先级更高）。"""
        root_md = tmp_path / "HCODE.md"
        root_md.write_text("root level", encoding="utf-8")
        dotdir = tmp_path / ".hcode"
        dotdir.mkdir()
        dot_md = dotdir / "HCODE.md"
    ...
    def test_dotdir_walks_up(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        """.hcode/HCODE.md 参与逐级遍历，深层目录的排在后面。"""
        subprocess.run(["git", "init"], cwd=tmp_path, capture_output=True)
        sub = tmp_path / "pkg" / "deep"
        sub.mkdir(parents=True)
        (tmp_path / ".hcode").mkdir(exist_ok=True)
        (tmp_path / ".hcode" / "HCODE.md").write_text("dotdir root", encoding="utf-8")
        (sub / ".hcode").mkdir()
        (sub / ".hcode" / "HCODE.md").write_text("dotdir leaf", encoding="utf-8")
...
class TestSession:
    def test_append_writes_jsonl_and_updates_meta(self, tmp_path: Path) -> None:
        sessions_dir = tmp_path / ".hcode" / "sessions"
...
class TestMemoryManager:
    ...
    def test_load_includes_memory_index(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        # 创建项目级记忆目录和文件
        project_mem_dir = tmp_path / "project" / ".hcode" / "memory"
    ...
    def test_load_all(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        # 用户级记忆
        user_mem_dir = fake_home / ".hcode" / "memory"
...
class TestConversationInjection:
    def test_inject_long_term_memory(self) -> None:
        ...
        assert "hcodeMd" in conv.history[1].content
    ...
    def test_inject_instructions_only(self) -> None:
        ...
        assert "hcodeMd" in conv.history[0].content
    ...
    def test_inject_skills_only(self) -> None:
        """项目可能没写 HCODE.md 也没有记忆，只有 Skill 时同样要注入。"""
        conv = ConversationManager()
...
class TestMemoryExtraction:
    def test_memory_types_aligned_with_go(self, tmp_path: Path) -> None:
        """验证四种记忆类型枚举。"""
        from hcode.memory.auto_memory import VALID_TYPES, _USER_LEVEL_TYPES, _PROJECT_LEVEL_TYPES
