...
class TestPathSandbox:
    ...
    def test_deny_write_paths(self) -> None:
        ...
        for rel in (
            ".hcode/permissions.local.yaml",
            ".hcode/config.yaml",
            ".hcode/skills/evil/SKILL.md",
        ):
    ...
    def test_temp_dir_allowed(self) -> None:
        tmp = Path(tempfile.gettempdir()) / "hcode_test.txt"
...
class TestRuleEngine:
    ...
    def test_append_local_rule(self) -> None:
        tmpdir = Path(tempfile.mkdtemp())
        local_path = tmpdir / ".hcode" / "permissions.local.yaml"
...
async def test_e2e_rule_allows_git():
    ...
    tmpdir = Path(tempfile.mkdtemp())
    rules_file = tmpdir / ".hcode" / "permissions.yaml"
