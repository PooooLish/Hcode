...
需要环境变量 HCODE_TEST_API_KEY、HCODE_TEST_BASE_URL、HCODE_TEST_MODEL。
...
def test_lock_first_acquire():
    from hcode.memory.consolidation import _read_last_consolidated_at, _try_acquire_lock
...
def test_lock_blocks_when_held():
    from hcode.memory.consolidation import _try_acquire_lock
...
def test_lock_rollback_deletes_on_zero():
    from hcode.memory.consolidation import _try_acquire_lock, _rollback_lock
...
def test_prompt_contains_all_phases():
    from hcode.memory.consolidation import _build_consolidation_prompt
...
def test_do_consolidation_builds_subagent(monkeypatch):
    ...
    from hcode.memory.consolidation import MemoryConsolidator
    ...
    with tempfile.TemporaryDirectory() as work_dir:
        mem_dir = os.path.join(work_dir, ".hcode", "memory")
...
def test_do_consolidation_subagent_inherits_rules(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as work_dir:
        mem_dir = os.path.join(work_dir, ".hcode", "memory")
        setup_test_memories(mem_dir)
        rules_file = Path(work_dir) / ".hcode" / "permissions.yaml"
...
def test_do_consolidation_subagent_opens_user_memory_dir(monkeypatch):
    ...
    with tempfile.TemporaryDirectory() as work_dir:
        ...
        fake_home = Path(os.path.abspath(os.sep)) / "fake-user-home-for-hcode-test"
        consolidator._user_mem_dir = str(fake_home / ".hcode" / "memory")
        ...
        user_mem_file = fake_home / ".hcode" / "memory" / "MEMORY.md"
...
@pytest.mark.skipif(
    not os.environ.get("HCODE_TEST_API_KEY"),
    reason="HCODE_TEST_API_KEY not set"
)
@pytest.mark.timeout(120)
def test_e2e_consolidation_merges_duplicates():
    api_key = os.environ["HCODE_TEST_API_KEY"]
    base_url = os.environ.get("HCODE_TEST_BASE_URL", "https://api.minimaxi.com/v1")
    model = os.environ.get("HCODE_TEST_MODEL", "MiniMax-M3")
...
async def _run_consolidation(work_dir, api_key, base_url, model, mem_dir):
    ...
    from hcode.permissions.checker import PermissionChecker
    from hcode.tools import ToolRegistry
    from hcode.tools.bash import Bash
    from hcode.tools.edit_file import EditFile
    from hcode.tools.glob import Glob
    from hcode.tools.grep import Grep
    from hcode.tools.read_file import ReadFile
    from hcode.tools.write_file import WriteFile
    ...
    from hcode.permissions.rules import RuleEngine
    from hcode.permissions.dangerous import DangerousCommandDetector
    from hcode.permissions.checker import PermissionMode
