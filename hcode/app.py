...
from hcode.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
)
from hcode import crashlog
from hcode.commands.completion import CompletionPopup
from hcode.commands.handlers import register_all_commands
from hcode.config import MCPServerConfig, ProviderConfig
from hcode.hooks import HookContext, HookEngine, load_hooks
from hcode.conversation import ConversationManager, Message
from hcode.mcp import ConnectResult, MCPManager
from hcode.mcp.tool_wrapper import mcp_tool_name_prefix
from hcode.memory import (
    MemoryManager,
    Session,
    SessionManager,
    RecallResult,
...
from hcode.agents.task_manager import TaskManager
from hcode.agents.trace import TraceManager
from hcode.agents.notification import inject_task_notifications
from hcode.commands.handlers.tasks import create_tasks_command
from hcode.skills.executor import SkillExecutor
from hcode.skills.loader import SkillLoader
from hcode.commands.handlers.skill_register import register_skill_commands
from rich.text import Text as RichText
from textual.theme import Theme
from hcode.tools import ToolRegistry, create_default_registry
from hcode.tools.agent_tool import AgentTool
from hcode.tools.ask_user import AskUserEvent, AskUserTool
from hcode.tools.impl.tool_search import ToolSearchTool
from hcode.tools.mcp_call import McpCallTool
from hcode.tools.install_skill import InstallSkillTool
from hcode.tools.load_skill import LoadSkill
from hcode.worktree.cleanup import start_stale_cleanup_task
from hcode.worktree.manager import WorktreeManager
from hcode.commands.handlers.worktree import create_worktree_command
from hcode.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".hcode", "build", ".gradle"}
...
class ConversationScroll(VerticalScroll):
    """Keep an existing end anchor through the next conversation resize."""

    def __init__(self, *children: Any, **kwargs: Any) -> None:
        super().__init__(*children, **kwargs)
        self._preserve_end_on_resize = False

    def preserve_end_on_next_resize(self) -> None:
        self._preserve_end_on_resize = self.scroll_y >= self.max_scroll_y

    def on_resize(self, _event: events.Resize) -> None:
        preserve_end = self._preserve_end_on_resize
        self._preserve_end_on_resize = False
        if preserve_end:
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".hcode" / "history"
...
class ToolCallBlock(Static, can_focus=True):
    ...
    def _render_loading(self) -> None:
        self.update(f"  ○ Running · {self._title}")
    ...
    def _render_collapsed(self) -> None:
        if self._is_error:
            self.update(f"  ✕ Failed · {self._title} ({self._elapsed:.1f}s)")
        else:
            self.update(f"  ✓ Done · {self._title} ({self._elapsed:.1f}s)")

    def _render_expanded(self) -> None:
        if self._is_error:
            header = f"  ✕ Failed · {self._title} ({self._elapsed:.1f}s)"
        else:
            header = f"  ✓ Done · {self._title} ({self._elapsed:.1f}s)"
...
class SubAgentBlock(Static, can_focus=True):
    ...
    def _render_running(self) -> None:
        ...
        self.update(f"○ Running · {self._agent_type}{desc}")
    ...
    def _render_done(self) -> None:
        ...
        state = "✕ Failed" if self._is_error else "✓ Done"
        if self._collapsed:
            self.update(
                f"{state} · {self._agent_type}{desc}\n"
                f"    ⎿  {tool_info}{self._elapsed:.1f}s  (ctrl+o to expand)"
            )
        else:
            self.update(
                f"{state} · {self._agent_type}{desc}\n"
                f"    ⎿  {tool_info}{self._elapsed:.1f}s\n"
...
class HcodeApp(App):
    ...
    BINDINGS = [
        ...
        Binding("f1", "show_shortcuts", "Help", priority=True),
    ]
    ...
    def __init__(
        ...
        from hcode.config import SandboxAppConfig
        ...
        self._runtime_error: bool = False
        self._active_teammates: int = 0
    ...
    def compose(self) -> ComposeResult:
        ...
        yield ConversationScroll(id="chat-area")
    ...
    def on_mount(self) -> None:
        self.register_theme(_HCODE_THEME)
        self.theme = "hcode"
        if len(self.providers) == 1:
    ...
    def on_resize(self, event: events.Resize) -> None:
        self.query_one("#chat-area", ConversationScroll).preserve_end_on_next_resize()
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        checker = PermissionChecker(
            ...
            rule_engine=RuleEngine(
                user_rules_path=home / ".hcode" / "permissions.yaml",
                project_rules_path=Path(work_dir) / ".hcode" / "permissions.yaml",
                local_rules_path=Path(work_dir) / ".hcode" / "permissions.local.yaml",
            ),
        ...
        if self._sandbox_cfg.enabled:
            from hcode.sandbox import SandboxConfig, create_sandbox
            os_sandbox = create_sandbox()
            if os_sandbox and os_sandbox.available():
                sandbox_config = SandboxConfig(
                    allow_write=[work_dir, "/tmp"],
                    deny_write=[
                        f"{work_dir}/.hcode/config.yaml",
                        f"{work_dir}/.hcode/permissions.local.yaml",
                    ],
        ...
        from hcode.filehistory import FileHistory
        ...
        from hcode.tools.exit_plan_mode import ExitPlanModeTool
        ...
        # --- Worktree 系统初始化 ---
        from hcode.config import WorktreeConfig
        ...
        from hcode.tools.enter_worktree import EnterWorktreeTool
        from hcode.tools.exit_worktree import ExitWorktreeTool
        ...
        # --- Agent 团队系统初始化 ---
        from hcode.teams.manager import TeamManager
        from hcode.tools.team_create import TeamCreateTool
        from hcode.tools.team_delete import TeamDeleteTool
        ...
        from hcode.commands.handlers.trace import create_trace_command
        ...
        from hcode.tools.send_message import SendMessageTool
        from hcode.tools.synthetic_output import SyntheticOutputTool
        from hcode.tools.task_stop import TaskStopTool
        ...
        if self._enable_coordinator_mode:
            from hcode.agents.tool_filter import apply_coordinator_filter
        ...
        work_dir = os.getcwd()
        self.query_one(WorkspaceHeader).set_context(
            os.path.basename(work_dir), provider.model
        )
    ...
    async def _send_message(self, text: str, is_notification: bool = False) -> None:
        ...
        await ai_row.mount(self._activity_indicator)
        ...
        try:
            async for event in self.agent.run(self.conversation):
                if isinstance(event, ThinkingText):
                    ...
                    if accumulated_text and streaming_label is not None:
                        ...
                        streaming_label = None
                        accumulated_text = ""
                    self._finish_activity(ActivityPhase.FAILED)
                    self._show_error(event.message)

                elif isinstance(event, LoopComplete):
                    total_time = _time.monotonic() - self._thinking_start
                    self._finish_activity(ActivityPhase.COMPLETED, total_time)
                    if self.session:
                        for msg in self.conversation.history[history_cursor:]:
                            self.session.append(msg)
                        history_cursor = len(self.conversation.history)
            ...
            self._finish_activity(ActivityPhase.CANCELLED)
            if accumulated_text:
                if streaming_label is not None:
                    await streaming_label.remove()
                md = Markdown(
    ...
    async def _show_plan_approval(self) -> None:
        from hcode.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        from hcode.plan_dialog import InlinePlanWidget, PlanChoice
        from hcode.prompts import build_plan_mode_exit_reminder
    ...
    async def _handle_askuser(self, event: AskUserEvent) -> None:
        from hcode.askuser_dialog import InlineAskUserWidget
    ...
    def _finish_activity(
        self, phase: ActivityPhase, elapsed: float | None = None
    ) -> None:
        activity = self._activity_indicator
        if activity is None or self._activity_terminal:
            return
        if elapsed is None:
            elapsed = max(0.0, _time.monotonic() - self._thinking_start)
        activity.finish(phase, elapsed)
        self._activity_terminal = True

    def _tick_spinner(self) -> None:
        """Refresh deterministic activity elapsed time and runtime status."""
        self._spinner_idx += 1
        elapsed = _time.monotonic() - self._thinking_start
        self._refresh_runtime_status()
        if self._activity_indicator is not None:
            self._activity_indicator.set_elapsed(elapsed)
    ...
    def _tick_teammate_tree(self) -> None:
        ...
        self._set_active_teammates(active_count)

    def _set_active_teammates(self, count: int) -> None:
        self._active_teammates = max(0, count)
        self._refresh_runtime_status()

    async def _handle_permission_request(self, request: PermissionRequest) -> None:
        from hcode.permission_dialog import InlinePermissionWidget
    ...
    async def _init_mcp(self) -> None:
        ...
        if self._selected_provider is not None:
            from hcode.mcp.loading_strategy import decide_and_apply
    ...
    def _runtime_snapshot(self) -> RuntimeSnapshot:
        ...
        return RuntimeSnapshot(
            ...
            elapsed=(
                max(0.0, _time.monotonic() - self._thinking_start)
                if self._streaming
                else 0.0
            ),
        )
