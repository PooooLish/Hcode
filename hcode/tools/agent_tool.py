...
class AgentTool(Tool):
    ...
    def _inherited_rule_engine(self) -> Any:
        ...
        父级没有权限检查器时退化为空规则集。
        """
        from hcode.permissions import RuleEngine
    ...
    async def execute(self, params: BaseModel) -> ToolResult:
        ...
        from hcode.agents.fork import ForkError, build_forked_messages
        from hcode.agents.parser import AgentDef
        from hcode.agents.tool_filter import clone_registry_for_fork, resolve_agent_tools
        from hcode.agent import Agent as AgentClass
    ...
    async def _execute_as_teammate(self, p: AgentToolParams) -> ToolResult:
        ...
        from hcode.agents.fork import ForkError, build_forked_messages
        from hcode.agents.parser import AgentDef
        from hcode.agents.tool_filter import build_teammate_tools
        from hcode.agent import Agent as AgentClass
        from hcode.conversation import ConversationManager
        from hcode.permissions import (
            DangerousCommandDetector,
        ...
        from hcode.teams.models import BackendType, TeammateInfo
    ...
    def _spawn_pane_teammate(
        ...
        from hcode.teams.spawn import build_teammate_cli
        ...
        if mailbox is not None and p.prompt:
            from hcode.teams.mailbox import create_message
            from hcode.teams.spawn_inprocess import LEAD_NAME
            mailbox.write(
                teammate_name,
                create_message(
        ...
        # 构造把本 hcode 拉起为队友 worker 模式的命令，cd 到该队友的 worktree
        cli_command = build_teammate_cli(p.team_name, teammate_name, wt.path)

        try:
            if backend == BackendType.TMUX:
                from hcode.teams.spawn_tmux import spawn_tmux_teammate
                ...
                from hcode.teams.spawn_iterm2 import spawn_iterm2_teammate
    ...
    async def _execute_with_worktree(self, p: AgentToolParams) -> ToolResult:
        ...
        from hcode.worktree.integration import (
    ...
    def _create_client_for_model(self, model_alias: str) -> LLMClient | None:
        ...
        from hcode.client import create_client
