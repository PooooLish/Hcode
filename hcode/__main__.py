...
from hcode.config import ConfigError, load_config
from hcode.hooks import HookConfigError, HookEngine, load_hooks
from hcode.permissions import PermissionMode


def main() -> None:
    # 队友 worker 模式：由 tmux/iTerm2 窗格用 `-m hcode --teammate ...` 拉起。
    ...
    # 先确保 .hcode/ 目录存在，否则下面写 debug.log 会因目录不存在而崩溃
    Path(".hcode").mkdir(parents=True, exist_ok=True)
    # 追加写：排查异常退出要看的往往是上一次运行的日志，覆盖会把现场冲掉
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(name)s %(message)s",
        filename=".hcode/debug.log",
        filemode="a",
    )
    crashlog.install()

    parser = argparse.ArgumentParser(prog="hcode", description="Hcode AI coding assistant")
    parser.add_argument(
        "--mode",
    ...
    if args.remote:
        from hcode.remote import RemoteServer
    ...
    from hcode.driver import NoAltScreenDriver
...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str, output_format: str = "text") -> None:
    ...
    from hcode.client import create_client, resolve_context_window
    from hcode.conversation import ConversationManager
    from hcode.memory.instructions import load_instructions
    ...
    if config.mcp_servers:
        from hcode.mcp import MCPManager
...
async def _build_teammate_registry(
    ...
    from hcode.tools.send_message import SendMessageTool
    from hcode.tools.synthetic_output import SyntheticOutputTool
    from hcode.tools.task_create import TaskCreateTool
    from hcode.tools.task_get import TaskGetTool
    from hcode.tools.task_list import TaskListTool
    from hcode.tools.task_update import TaskUpdateTool
    from hcode.worktree import WorktreeManager
...
async def _run_teammate(team_name: str, agent_name: str) -> None:
    ...
    from hcode.teams.spawn_inprocess import LEAD_NAME, spawn_inprocess_teammate
    ...
    checker = PermissionChecker(
        ...
        sandbox=PathSandbox(work_dir),
        rule_engine=RuleEngine(
            user_rules_path=Path.home() / ".hcode" / "permissions.yaml",
            project_rules_path=Path(work_dir) / ".hcode" / "permissions.yaml",
            local_rules_path=Path(work_dir) / ".hcode" / "permissions.local.yaml",
        ),
