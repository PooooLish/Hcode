...
def _session_path(hcode_dir: Path) -> Path:
    return hcode_dir / SESSION_FILENAME
...
def load_worktree_session(hcode_dir: Path) -> WorktreeSession | None:
    path = _session_path(hcode_dir)
    if not path.exists():
        return None
    try:
