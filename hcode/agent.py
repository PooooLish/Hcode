...
from hcode.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from hcode.conversation_pairing import REJECTED_TOOL_RESULT
from hcode.conversation import ThinkingBlock as ConvThinkingBlock
from hcode.memory.auto_memory import MemoryManager
from hcode.permissions import (
    Decision,
    PermissionChecker,
    PermissionMode,
)
from hcode.hooks import HookContext, HookEngine, ToolRejectedError
from hcode.hooks.engine import HookNotification
from hcode.prompts import build_environment_context, build_plan_mode_reminder, build_system_prompt
...
class Agent:
    ...
    def _announce_deferred_tools(self, conversation: ConversationManager) -> None:
        ...
        from hcode.mcp.loading_strategy import McpLoadingMode
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".hcode" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".hcode" / "plans"
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from hcode.permissions.rules import Rule, extract_content
    ...
    def _maybe_persist_or_truncate(
        ...
        from hcode.context.manager import (
