...
"""从 .hcode/commands/ 目录加载自定义 Markdown 命令。
...
from hcode.commands.registry import Command, CommandContext, CommandType
...
def load_user_commands(work_dir: str) -> list[Command]:
    ...
    搜索路径（后者覆盖前者）：
      1. ~/.hcode/commands/
      2. <work_dir>/.hcode/commands/
    """
    dirs: list[str] = []
    home = Path.home()
    dirs.append(str(home / ".hcode" / "commands"))
    dirs.append(str(Path(work_dir) / ".hcode" / "commands"))
