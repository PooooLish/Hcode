...
def load_instructions(project_root: str) -> str:
    ...
    发现顺序（低优先级在前，高优先级在后）：
    1. 用户全局：~/.hcode/HCODE.md, ~/.hcode/AGENTS.md
    2. 项目目录链：从 git root 到 workDir，每个目录的 HCODE.md、AGENTS.md
       和 .hcode/HCODE.md
    3. workDir/HCODE.local.md（本地覆盖）
    """
    ...
    # 1. 用户全局
    _add(home / ".hcode" / "HCODE.md")
    _add(home / ".hcode" / "AGENTS.md")

    # 2. 项目目录链
    for d in _project_instruction_dirs(root):
        _add(d / "HCODE.md")
        _add(d / "AGENTS.md")
        # .hcode/ 下的同名文件：想让指令进 .gitignore 的项目放这里
        _add(d / ".hcode" / "HCODE.md")

    # 3. 本地覆盖
    _add(root / "HCODE.local.md")
